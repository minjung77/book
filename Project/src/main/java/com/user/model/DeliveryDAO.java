...
public class DeliveryDAO {
	...
	// DeliverYDAO 클래스를 싱글턴 방식으로 만들어 보자.
	...
	// 2단계 : DeliverYDAO 클래스를 정적(static) 멤버로 
	//        선언을 해 주어야 한다.
	private static DeliveryDAO instance;
	
	private DeliveryDAO() {  }  // 기본 생성자
	...
	public void insertDelivery(DeliveryDTO deliveryDTO) {
	    ...
	    try {
	        // 배송 ID를 수동으로 생성 (현재 가장 큰 delivery_id에 1을 더함)
	        String sql = "SELECT MAX(delivery_id) FROM delivery";
	        pstmt = con.prepareStatement(sql);
	        rs = pstmt.executeQuery();
	        int deliveryId = 0;
	        if (rs.next()) {
	            deliveryId = rs.getInt(1) + 1;  // 가장 큰 delivery_id에 1을 더하여 새 ID를 생성
	        } else {
	            deliveryId = 1;  // 만약 데이터가 없다면 1부터 시작
	        }

	        // 배송 정보 삽입 (DELIVERY_END 컬럼을 제외)
	        sql = "INSERT INTO delivery (delivery_id, order_id, user_id, address_1, address_2, delivery_start) "
	              + "VALUES (?, ?, ?, ?, ?, ?)";
	        pstmt = con.prepareStatement(sql);
	        pstmt.setInt(1, deliveryId);
	        pstmt.setInt(2, deliveryDTO.getOrderId());
	        pstmt.setString(3, deliveryDTO.getUserId());
	        pstmt.setString(4, deliveryDTO.getAddress1());
	        pstmt.setString(5, deliveryDTO.getAddress2());
	        pstmt.setDate(6, new java.sql.Date(deliveryDTO.getDeliveryStart().getTime()));
